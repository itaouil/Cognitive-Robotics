    ...
    def correction_step(self, sensor_measurements):
        ...
        for measurement in sensor_measurements:
            ...
            if not landmark.observed:
                # Compute angle of landmark seeing
                angle = normalize_angle(robot_pose[2] + measurement.z_bearing)

                # Compute x and y of landmark
                x = robot_pose[0] + measurement.z_range * np.cos(angle)
                y = robot_pose[1] + measurement.z_range * np.sin(angle)
                ...
                H_inv_T = H_inv.T
                landmark.sigma = np.dot(np.dot(H_inv, Q_t), H_inv_T)
                ...
                first_term = np.dot(landmark.sigma, H.T)
                second_term = np.dot(np.dot(H, landmark.sigma), H.T) + Q_t
                second_term_inv = np.linalg.inv(second_term)
                k_gain = np.dot(first_term, second_term_inv)

                # Compute the error between the z and expected_z (remember to normalize the angle)
                meas = np.zeros((2, 1))
                meas[0, 0] = measurement.z_range
                meas[1, 0] = measurement.z_bearing
                z_error = meas - expected_z
                z_error[1] = normalize_angle(z_error[1])

                # Update the mean and covariance of the EKF for this landmark
                landmark.mu = landmark.mu + np.dot(k_gain, z_error)
                landmark.sigma = np.dot((np.identity(2) - np.dot(k_gain, H)), landmark.sigma)

                # Compute the likelihood of this observation, multiply with the former weight
                # to account for observing several features in one time step
                cost = np.power(2*np.pi*np.linalg.det(Q_t), -1/2)
                exp_ = np.exp(-0.5*np.dot(np.dot(z_error.T, np.linalg.inv(Q_t)), z_error))
                prob = cost * exp_
                # dist_prob = random.gauss(np.abs(measurement.z_range - expected_z[0]), self.noise[1])
                # bear_prob = random.gauss(np.abs(measurement.z_bearing - expected_z[1]), self.noise[0])
                # prob = dist_prob * bear_prob
                self.weight *= prob
...
def main():
    ...
    landmarks_truth = read_world('/Users/dailand10/Desktop/Cognitive-Robotics/fast-slam/data/world.dat')

    # Read sensor readings, i.e. odometry and range-bearing sensor
    data = read_sensor_data('/Users/dailand10/Desktop/Cognitive-Robotics/fast-slam/data/sensor_data.dat')
